...
import { callArk, parseArkJson } from "../llm/arkClient.js";
...
export async function generateComment(post: ScoredPost, note: Note | null): Promise<string> {
  if (process.env.COMMENT_PROVIDER === "ark") {
    const content = await callArk(
      ...
      { modelKind: "default", temperature: 0.72, maxTokens: 220, label: "Ark comment generation" }
    );
...
async function generateInteractionWithArk(post: ScoredPost, notes: Note[]): Promise<InteractionDraft> {
  const content = await callArk(
    ...
    { modelKind: "default", temperature: 0.72, maxTokens: 220, label: "Ark interaction agent" }
  );

  const parsed = parseArkJson<{ noteId?: number | string | null; comment?: string; rationale?: string }>(
    content,
    "Ark interaction agent"
  );
